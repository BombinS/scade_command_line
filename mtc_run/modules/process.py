...
def get_command(scade_bin, root_model, procedure):
    ...
    result_directory = os.path.join(result_directory,'QTE_MTC')
    scade_executable = os.path.join(scade_bin, 'QTE.exe')
    command = f'\"{scade_executable}\" -mtc -conf "KCG" -test_file \"{procedure}\" -target_dir \"{result_directory}\" \"{root_model}\"'
    return command
